box(title = "Input", width = 3, solidHeader = T, status = "primary", 
    fluidRow(
      ...
      conditionalPanel(
        ...
        conditionalPanel(
          condition = "input.fill_table",
                        
            column(6, p(HTML("<b>Initial conc. (M)</b>"),
                        numericInput('initial_denaturant',NULL, 6,min = 1, max = 12))),

            column(6, p(HTML("<b># Replicates</b>"),numericInput('n_replicates',NULL, 2,min = 1, max = 6))),

            column(6, p(HTML("<b>Dil. factor</b>"),numericInput('dil_factor',NULL, 2,min = 1, max = 10))),

            column(6, p(HTML("<b>Reverse order</b>"),checkboxInput("rev_order", "", FALSE)))
            ),
        ...
        column(6, p(HTML("<b>Use SI units</b>"),
                    span(shiny::icon("info-circle"), id = "info_uu1-6"),
                    checkboxInput("use_SI_units", NULL, FALSE),
                    tippy::tippy_this(
                      elementId = "info_uu1-6",
                      tooltip = "If selected, CheMelt will use Kelvin for temperature
                      and kilojoules for energy.
                      Otherwise, CheMelt will use degrees Celsius for temperature and kilocalories for energy.",
                      placement = "right")))
